 # cant get out if stdev is beyond something
...
def farzam_indicators(open, high, low, close, parameters_input):
    ...
    stdev_exit = parameters_input['stdev_exit']
    count = 0
    in_position = False
    long_top = 0
    long_bot = 0
    ...
    conditions = pd.concat([long_entry_cond, short_entry_cond, top, bot, diff, close,stdev20], axis=1).rename(columns={0: "long_entry_cond", 1: "short_entry_cond", 2: "top", 3: "bot", "close_price": "close"})
...
long_ma_max  = [1000]
short_ma_min = [-20]
stdev_exit = [55,60,65,70,75,80,85,90,95,100,105]
