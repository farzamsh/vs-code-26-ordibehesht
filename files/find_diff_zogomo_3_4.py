...
class Zogomo3(Strategy):
    ...
    def __init__(strategy, data, long_ent_type, long_close_type, short_ent_type, short_close_type, hl_len, hi_ma_len,
                 sl_input, hma_len, hma2_len, long_bool=True,
                 short_bool=True) -> None:
        super().__init__()
        ...
        strategy.set_parameters(long_ent_type=long_ent_type, long_close_type=long_close_type,
                                ...
                                long_bool=long_bool, short_bool=short_bool)
        strategy.run()
    ...
    def indicators(strategy) -> None:
        ...
        hma576 = Indicator("hma576", ta.hma, args=(hlcc4, hma_len))

        # hma2_len = 70
        hma2_len = strategy.hma2_len
        hma2 = Indicator("hma2", ta.hma, args=(hlcc4, hma2_len))
        strategy.add(hma576, hma2)
    ...
    def condition(strategy):
        ...
        strategy.hma576 = strategy.hma576.round(decimals=2)
        hma2_change_long = strategy.hma2 > strategy.hma2.shift()
        hma2_change_short = strategy.hma2 < strategy.hma2.shift()

        hma_change_long = strategy.hma576 > strategy.hma576.shift()
        hma_change_short = strategy.hma576 < strategy.hma576.shift()

        strategy.pervious_low = 0
        strategy.pervious_high = 0
        longEntryCondition = (long_ent & hma_change_long & hma2_change_long).rename("long_ent")
        longCloseCondition = long_close.rename("long_close")
        shortEntryCondition = (short_ent & hma2_change_short & hma_change_short).rename("short_ent")
        ...
        strategy.conditions = longEntryCondition, longCloseCondition, shortEntryCondition, shortCloseCondition, strategy.highest_ma.rename(
            ...
            "hl_diff"), hma2_change_long.rename("hma2_change_long"), hma2_change_short.rename("hma2_change_short"), hma_change_long.rename("hma_change_long"), hma_change_short.rename("hma_change_short")
    ...
    def trade_calc(strategy, row):
        ...
        long_greedy = (1 <= long_profit <= 5) and row.hma2_change_short
        long_run_away = strategy.longEntryPrice != -1.0 and (row.close - strategy.longEntryPrice > 0) and row.hma_change_short
        long_constant_sl = long_profit < -5

        short_profit = (strategy.shortEntryPrice - row.close) * 100 / strategy.shortEntryPrice
        short_greedy = (1 <= short_profit <= 5) and row.hma2_change_long
        short_constant_sl = short_profit < -5
        # to get closer profits to run-up || 1 must be replaced with avg trade % and 3 with WHAT?!
        short_run_away = strategy.shortEntryPrice != -1.0 and row.close - strategy.shortEntryPrice < 0 and row.hma_change_long
        if strategy.short_bool:
            if row.short_close or short_sl or short_run_away or short_greedy or short_constant_sl:
                if strategy.shortEntryPrice != -1.0:
                    strategy.exit("short", signal=f'{row.short_close=} {short_sl=} {short_run_away=} {short_greedy=}')
                    strategy.shortEntryPrice = -1.0
                    strategy.shortSL = 1000000.0
        ...
        if strategy.long_bool:
            if row.long_close or long_sl or long_run_away or long_greedy or long_constant_sl:
                if strategy.longEntryPrice != -1.0:
                    strategy.exit("long", signal=f'{row.long_close=} {long_sl=} {long_run_away=} {long_greedy=}')
                    strategy.longEntryPrice = -1.0
                    strategy.longSL = 0
