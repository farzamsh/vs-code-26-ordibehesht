 # in version 1,this is the most default version that works perfectly
...
def farzam_indicators(open, high, low, close, parameters_input):
    ...
    def condition_trade(row, take_profit_long,take_profit_short,stop_loss_long,stop_loss_short):
        ...
        if row.long_entry_cond and not in_position:
            ...
            #tp = {row.bot +take_profit_long*row.diffrent_top_bot },\
                                 #sl = {row.bot - stop_loss_long * row.diffrent_top_bot},

            strategy.entry(row.name, "long", "long",comment = f"top ={row.top:.2f} , bot ={row.bot:.2f} , \
                            stop_loss = {stop_loss_long} ,tp = {take_profit_long} , diff ={row.diffrent_top_bot:.2f} , close = {row.close} , stdev20 ={row.stdev20:.2f} , counter ={row.counter}, (C > sma): {sma_is_h}")
            in_position = True
            long_top = row.top
            long_bot = row.bot
        ...
        if row.short_entry_cond and not in_position:
            ...
            #tp = {row.top - take_profit_short*row.diffrent_top_bot}, \
                                 #sl = {row.top + stop_loss_short*row.diffrent_top_bot}
            strategy.entry(row.name, "short", "short",comment = f"top ={row.top:.2f} , bot ={row.bot:.2f} , \
                             diff ={row.diffrent_top_bot:.2f} , stop_loss = {stop_loss_short} , tp ={take_profit_short} , close = {row.close} , stdev20 ={row.stdev20:.2f} , counter ={row.counter} ,( C < sma): {sma_is_l}")
            in_position = True
            short_top = row.top
            short_bot = row.bot
